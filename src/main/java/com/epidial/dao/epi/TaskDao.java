...
public interface TaskDao {
    ...
    @Insert("insert into task" +
                "(uid ,gid,name,imgpath,pay,orderno,type,valid,total,createTime,addrid,delivery,payTime,email,barcode,prom) " +
            "values " +
                "(#{uid},#{gid},#{name},#{imgpath},#{pay},#{orderno},#{type},#{valid},#{total},#{createTime},#{addrid},#{delivery},#{payTime},#{email},#{barcode},#{prom})")
    ...
    @Delete("delete from task where uid=#{uid} and pay=0 and type=#{type}")
    public void  deleteUnPayGoods(@Param("uid") Object uid,@Param("type") Object type);
   ...
   @Update("update task set " +
                ...
                "email=#{email},barcode=#{barcode},prom=#{prom} where id=${id} ")
   public void  update(Task task);

   @Select("select * from task where id=#{id}")
   public Task findById(@Param("id") Object id);

   @Select("select sum(total) from task where pay=0 and valid=0 and type=0 and uid=#{uid}")
   public float findUnPayDnaKitTotalPrice(@Param("uid") Object uid);

    @Select("select sum(total) from task where pay=0 and valid=0 and type=1 and uid=#{uid}")
    public float findUnPayNoDnkKitTotalPrice(@Param("uid") Object uid);
